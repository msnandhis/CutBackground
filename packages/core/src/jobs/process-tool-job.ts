...
import {
  createBackgroundRemovalPrediction,
...
import { logToolUsage } from "../usage-logging";
...
async function deliverClientWebhook(
  ...
  const webhookUrl =
    typeof metadata.clientWebhookUrl === "string"
      ? metadata.clientWebhookUrl
      : null;
  const webhookSecret =
    typeof metadata.clientWebhookSecret === "string"
      ? metadata.clientWebhookSecret
      : null;
...
async function finalizeToolJobFailure(
  ...
  if (updatedJob) {
    ...
    await logToolUsage({
      userId: updatedJob.userId,
      toolName: "background-remover",
      action: nextStatus === "canceled" ? "job_canceled" : "job_failed",
      metadata: {
        jobId: updatedJob.id,
        errorMessage: params.errorMessage,
      },
...
export async function processToolJobExecution(jobId: string) {
  ...
  if (job.status === "succeeded") {
    logger.info({ jobId }, "Job already succeeded, skipping processing.");
    return;
  }

  if (job.status === "failed") {
    logger.info({ jobId }, "Job already failed, skipping processing.");
    return;
  }
  ...
  if (
    ...
    logger.warn(
      ...
      "Job has unexpected status, skipping processing.",
    );
    return;
  }
...
export async function completeToolJobFromReplicateWebhook(
  prediction: ReplicatePredictionPayload,
) {
  const job = await db.query.jobs.findFirst({
    where: (table, { eq: equals }) =>
      equals(table.providerJobId, prediction.id),
  });
