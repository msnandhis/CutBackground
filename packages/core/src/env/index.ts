...
const defaultToolStorageRoot = path.resolve(
  moduleDir,
  "../../../../.runtime/tool-assets",
);
...
const envSchema = z.object({
  ...
  REPLICATE_WEBHOOK_TOLERANCE_SECONDS: z.coerce
    .number()
    .int()
    .positive()
    .optional(),
  ...
  TOOL_STALE_JOB_THRESHOLD_SECONDS: z.coerce
    .number()
    .int()
    .positive()
    .optional(),
...
export function getAuthSecret() {
  ...
  console.warn(
    "BETTER_AUTH_SECRET is not set. Authentication will fail. " +
...
export function getBackgroundQueueDependencyStatus(): RuntimeDependencyStatus {
  ...
  const missingEnv = [
    ...(parseQueueEnabledFlag(env.ENABLE_BACKGROUND_QUEUE)
      ? []
...
export function getReplicateConfig() {
  ...
  return {
    ...
    model:
      env.REPLICATE_BACKGROUND_REMOVER_MODEL || "851-labs/background-remover",
...
export function getEmailConfig(): {
  ...
  if (
    (selectedProvider === "auto" || selectedProvider === "resend") &&
    env.RESEND_API_KEY
  ) {
    return {
      provider: "resend",
  ...
  if (
    (selectedProvider === "auto" || selectedProvider === "brevo") &&
    env.BREVO_API_KEY
  ) {
    return {
      provider: "brevo",
...
export function assertRuntimeRequirements(target: RuntimeDependency) {
  const status = getRuntimeDependencyStatuses().find(
