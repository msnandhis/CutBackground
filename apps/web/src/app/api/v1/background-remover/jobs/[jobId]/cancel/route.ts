...
import { cancelToolJob } from "@/features/tool/lib/server/tool-jobs";
...
export async function POST(
    ...
    try {
        ...
        await enforceRateLimit({
            identifier: `api-key:${principal.apiKeyId}:cancel-job`,
            maxRequests: 30,
            windowSeconds: 60,
            code: "RATE_LIMITED",
            message: "Too many cancel requests in a short period.",
        });

        const { jobId } = parseRouteParams(await params, toolJobRouteParamsSchema);
        const job = await cancelToolJob(jobId, principal.userId);
