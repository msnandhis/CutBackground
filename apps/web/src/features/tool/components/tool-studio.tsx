                    ...
                    {!sessionData?.user && !isSessionPending ? (
                        <div className="rounded-[1.5rem] bg-amber-50 p-6 text-amber-950">
                            ...
                            <p className="mt-3 text-sm leading-relaxed">
                                Background removal now creates real jobs, outputs, and dashboard history, so an
                                authenticated workspace session is required.
                            </p>
                            <div className="mt-6">
                    ...
                    {state === "empty" ? (
                        <div className="text-center">
                            ...
                            <p className="mt-3 text-neutral-600">
                                Upload a product, portrait, or marketing asset and we will create a persisted job,
                                track execution, and keep the result available in your dashboard.
                            </p>
                    ...
                    {state === "uploading" || state === "processing" ? (
                        <div className="space-y-5">
                            <div className="flex items-center justify-between text-sm text-neutral-600">
                                <span>{state === "uploading" ? "Uploading image" : "Processing result"}</span>
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                <div className="rounded-2xl bg-white p-4">
                                    <p className="text-sm font-semibold text-brand-dark">Current file</p>
                                    <p className="mt-2 text-sm text-neutral-500">
                                        {selectedFile?.name ?? "No file selected"}
                                    </p>
                                </div>
                                <div className="rounded-2xl bg-white p-4">
                                    <p className="text-sm font-semibold text-brand-dark">Job status</p>
                                    <p className="mt-2 text-sm text-neutral-500">
                                ...
                                >
                                    {job?.outputUrl ? (
                                        ...
                                        <div className="text-sm text-neutral-600">
                                            Output storage is ready, but this run did not expose a preview URL.
                                        </div>
                                    )}
                                </div>
                            </div>
            ...
            <div className="space-y-6">
                <div className="rounded-[2rem] border border-neutral-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-5 space-y-3">
                        {addons.map((addon) => (
                            ...
                            >
                                <span>{addon}</span>
                                <span className="text-xs uppercase tracking-wide text-neutral-400">next</span>
                            </button>
                        ))}
                    </div>
                </div>
                ...
                <div className="rounded-[2rem] border border-neutral-200 bg-brand-dark p-6 text-white shadow-sm">
                    ...
                    <ul className="mt-5 space-y-3 text-sm text-white/75">
                        <li>Validated uploads create real job records and durable output assets</li>
                        <li>Dashboard history, retry, cancel, and API key access share the same job contract</li>
                        <li>Webhook-enabled provider execution can complete asynchronously without blocking the request</li>
                        <li>Public API consumers can receive completion callbacks and signed webhook headers</li>
                        <li>Add-on controls are staged here so the next feature pass lands on the same result surface</li>
                    </ul>
                </div>
            </div>
        </div>
